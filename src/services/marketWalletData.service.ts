...
export async function getMarketWalletData(walletAddress: string, marketName: string, assetName: string, oracleName: string) {
  ...
  const marketContract = new CompoundMarketContract(provider, metadata[marketName]["ABI"], metadata[marketName]["address"]);
  const assetContract = new AssetContract(provider, metadata[assetName]["ABI"], metadata[assetName]["address"]);
  const oracleContract = new AssetOracleContract(provider, metadata[oracleName]["ABI"], metadata[oracleName]["address"]);
